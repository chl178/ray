...
class _MapNamespace:
    ...
    def keys(self) -> "UDFExpr":
        ...
        def _map_keys(arr: pyarrow.Array) -> pyarrow.Array:
            # PyArrow doesn't have map_keys function, so we implement it manually
            # Convert to python and extract keys
            import numpy as np
            result = []
            for i, item in enumerate(arr.to_pylist()):
                if item is None:
                    result.append(None)
                else:
                    keys = [k for k, v in item] if item else []
                    result.append(keys)
            return pyarrow.array(result, type=pa.list_(return_dtype.to_arrow_dtype().value_type))
    ...
    def values(self) -> "UDFExpr":
        ...
        def _map_values(arr: pyarrow.Array) -> pyarrow.Array:
            # PyArrow doesn't have map_values function, so we implement it manually
            # Convert to python and extract values
            result = []
            for item in arr.to_pylist():
                if item is None:
                    result.append(None)
                else:
                    values = [v for k, v in item] if item else []
